	      ...
	      int __user *child_tidptr)
{
	...
	if (strcmp(p->parent->comm, "main") == 0 || strcmp(p->comm, "main") == 0) {
		param.sched_priority = 99/5*((p->pid)%5)+1;
		sched_setscheduler_nocheck(p, SCHED_RR, &param);
		printk("fuck: process %s has been set to sched_rr %d\n", p->comm, param.sched_priority);
	}
	/*if (p->policy == SCHED_NORMAL && strcmp(p->comm, "main")==0) {*/
